...
class TestUser(APITestCase):
    ...
    def test_user_token_verification(self):
        response = self.client.post("/api/token/verify/", {"token": self.access_token})
    ...
    def test_user_token_refresh(self):
        ...
        try_old_access_token = self.client.post(
            "/api/token/verify/", {"token": self.access_token}
        )
        # This will pass because the life span of access token is 15 min.
        self.assertEqual(try_old_access_token.status_code, 200)

        try_new_access_token = self.client.post(
            "/api/token/verify/", {"token": new_access_token}
        )
